...
class Pagination 
{
	...
	function create_links()
	{
		...
		if ($currentPage > 1){
			$output .= sprintf('<li class="previous"><a href="' . $link . '"><i class="fa fa-chevron-left"></i></a></li>', $currentPage - 1);
		}
		...
		if ($currentPage < $totalPages){
			$output .= sprintf('<li class="next"><a href="' . $link . '"><i class="fa fa-chevron-right"></i></a></li>', $currentPage + 1);
		}
		...
		return '<ul class="pagination fr">' . $output . '</ul>';
	}
}
